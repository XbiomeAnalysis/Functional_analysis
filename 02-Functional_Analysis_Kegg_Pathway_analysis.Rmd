...
### Remove description in KEGG map profile

Here we need to remove the description column of kegg map profile of 8 samples **in Bash**. But if you have removed the column data already, please skip this chunk and jump to [Merge KEGG map profile of samples](#merge-kegg-map-profile-of-samples).

```mamrkdown
for i in /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/PipelineOutput/*/humann2_post_kegg/*_pathway_abundance_median_des.tsv; do awk -F '\t' '{print $1"\t"$2}' $i > $i.tmp.tsv ;done
```

### Merge KEGG map profile of samples  

Here we need to merge all data from 8 samples into one profile table with the merge script from Humann2 **in Bash**. But if you have merged the sample data already, please skip this chunk and jump to [Data loading](#data-loading-kegg).

```markdown
docker run -i --rm -u $(id -u):$(id -g) -v /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/:/in harbor.xbiome.com/xbiome/environments/humann2:2.8.1-2b8c5c3 bash -c "merge_metaphlan_tables.py /in/PipelineOutput/*/humann2_post_kegg/*.tmp.tsv > /in/Kegg/merged_kegg_profile.tsv"
```
...
```markdown
rm /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/PipelineOutput/*/humann2_post_kegg/*.tmp.tsv

sed -i '/pathway\t/d' /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/Kegg/merged_kegg_profile.tsv
```
...
metadata %<>% mutate(SeqID2 = SeqID) %>% column_to_rownames('SeqID2') %>% as.data.frame()
```

---

## Data preprocessing (KEGG)

### Remove 0 abundance maps  

**Remove the pathways whose abundance are 0 across all samples.** _If we use harmonic mean or median to calcualte pathway abundance from KOs, the pathway abundance may be 0, we therefore need to remove those pathways having 0 abundance across all samples._
...
dim(pathway_profile_tbl)
```

### Transforming data (KEGG)

**Note: Run [Remove 0 abundance maps](#remove-0-abundance-maps) before running this chunk!**  
...
dim(rescaled_pathway_profile_tbl)
```
...
Note: Run [Transforming data](#transforming-data-kegg) before running this chunk!
...
## Standard Analysis (KEGG)

**Note: All chunks in [Data preprocessing](#data-preprocessing-kegg) should be excuted before doing analysis in this chunk.**
...
```{r Compositional barplot of kegg pathway, fig.width = 6, fig.height = 9,include=FALSE, echo=FALSE}
## In this chunk, we construct stacked pathway barplot to depict the pathway composition of samples, we use function plot_stacked_bar from xviz to plot.
## In case there are too much entries, we use parameter "collapse" in plot_stacked_bar function to integrate entries whose abundance are below given threshold into "Others".
## Note: make sure your graph is long enough to show the group information.
...
Plot compositional barplot
```{r Plot compositional barplot}
...
## Differential analysis (KEGG)

**Note: All chunks in [Data preprocessing](#data-preprocessing-kegg) should be executed before doing analysis in this chunk.**

### Filter low prevalence pathway (KEGG)

Low prevalence pathways are pathways only occur in minor samples.
In this chunk, we would remove pathways apperaing in less than max(2 , 5% of samples) from data set before doing analysis. **If you have filtered the low prevalence pathways please jump to [DA with Logistic Regression](#da-with-logistic-regression) for differential analysis.
...
### DA with Logistic Regression

_In this chunk, you would be using logistic regression model to find pathways that are significantly enriched in certain group._  

**Note: Remeber to [Filter low prevalence pathway](#filter-low-prevalence-pathway-kegg) before running this chunk.**
...
```markdown
/share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/PipelineOutput/*/humann2_post_kegg/16466_filter_unstratified_with_des.kegg
```

##### Remove description in KO profile

Here we need to remove the description column of KO profile of 8 samples **in Bash**. But if you have removed the column data already, please skip this chunk and jump to [Read merged KO profile](#read-merged-ko-profile).

```mamrkdown
for i in /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/PipelineOutput/*/humann2_post_kegg/*filter_unstratified_with_des.kegg; do awk -F '\t' '{print $1"\t"$2}' $i > $i.tmp.tsv ;done
```

##### Merge KO profile of all sample

Here we need to merge all KO data from 8 samples into one profile table with the merge script from Humann2 **in Bash**. But if you have merged the sample data already, please skip this chunk and jump to [Read merged KO profile](#read-merged-ko-profile).

```markdown
docker run -i --rm -u $(id -u):$(id -g) -v /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/:/in harbor.xbiome.com/xbiome/environments/humann2:2.8.1-2b8c5c3 bash -c "merge_metaphlan_tables.py /in/PipelineOutput/*/humann2_post_kegg/*.tmp.tsv > /in/Kegg/merged_ko_profile.tsv"
```

##### Remove tmp KO tsv files
...
sed -i '/filter_unstratified_with_des.kegg.tmp/d' /share/projects/SOP/Functional_Analysis/Tongbangzhuo/Demodata/Kegg/merged_ko_profile.tsv
```

#### Read merged KO profile
...
colnames(KO_profile) %<>% str_remove('_.+')

```

#### Preprocess merged KO profile

```{r preprocess KO profile data}
## Rescale KO profile
...
#### Find differential KO for each differential Kegg map

```{r Find differential KO for each differential Kegg map}
