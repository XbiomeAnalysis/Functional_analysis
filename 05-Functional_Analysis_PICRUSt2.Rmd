...
In this chunk, we use TSS (Total sum scaling) to eliminate the influence of sequencing depth on samples.  Alternatively, you can use rarefied 16s data as input for Picrust2 in [Run PICRUSt2](#run-picrust2) to eliminate the sequencing depth differences among samples. If rarefied 16s data is used, please jump to [Filter low prevalence pathway](#filter-low-prevalence-pathway) chunk directly.
