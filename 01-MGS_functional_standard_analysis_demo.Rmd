...
(This script was written in _**/share/projects/SOP/Functional_Analysis/Tongbangzhuo/Phase1/Metacyc/**_ , all relative paths below are under this complete path.)
...
metadata <- read.table("./Demodata/metadata.txt",header = TRUE,stringsAsFactors = FALSE) %>%
...
pathwaydata <- read_delim("./Demodata/merged_relab_pathabundance_unstratified.tsv", delim = "\t") %>%
...
pathway_stratified <- read_delim("./Demodata/merged_relab_pathabundance_stratified.pcl", delim = "\t") %>%
...
read_delim("./Demodata/merged_relab_pathabundance_stratified.tsv", delim = "\t") %>%
    column_to_rownames("# Pathway") %>%
    t() %>%
    as.data.frame() %>%
